(() => {
  ...
  function u(...s) {
    return new URL(S(...s), document.baseURI).toString();
  }
  function S(...s) {
    return s
      .filter((t) => !!t)
      .map((t, i) => (i === 0 ? _(t) : j(_(t))))
      .filter((t) => t.length)
      .join("/");
  }
  function j(s) {
    let t = 0;
    for (; t < s.length && s.charAt(t) === "/";) t++;
    return s.substring(t);
  }
  function _(s) {
    let t = s.length;
    for (; t > 0 && s.charAt(t - 1) === "/";) t--;
    return s.substring(0, t);
  }
  function b(s, t) {
    return s.canvasKitBaseUrl
      ? s.canvasKitBaseUrl
      : t.engineRevision && !t.useLocalCanvasKit
        ? S("https://www.gstatic.com/flutter-canvaskit", t.engineRevision)
        : "canvaskit";
  }
  var h = class {
    constructor() {
    ...
    async load(t, i, r, e, n) {
      n ??= (o) => {
        o.initializeEngine(r).then((l) => l.runApp());
      };
    ...
    didCreateEngineInitializer(t) {
      typeof this._didCreateEngineInitializerResolve == "function" &&
        (this._didCreateEngineInitializerResolve(t),
    ...
    _loadJSEntrypoint(t, i, r) {
      ...
      if (!this._scriptLoaded) {
        ...
        let n = this._createScriptTag(t, r);
        if (e)
    ...
    async _loadWasmEntrypoint(t, i, r, e) {
      if (!this._scriptLoaded) {
        ...
        let { mainWasmPath: n, jsSupportRuntimePath: a } = t,
          o = u(r, n),
          l = u(r, a);
        this._ttPolicy != null && (l = this._ttPolicy.createScriptURL(l));
        let m = WebAssembly.compileStreaming(fetch(o)),
          c = await import(l),
          w;
        t.renderer === "skwasm"
          ? (w = (async () => {
            let d = await i.skwasm;
            return (
              (window._flutter_skwasmInstance = d),
              {
                skwasm: d.wasmExports,
                skwasmWrapper: d,
                ffi: { memory: d.wasmMemory },
              }
            );
          })())
          : (w = {});
        let f = await c.instantiate(m, w);
        await c.invoke(f);
      }
    }
    _createScriptTag(t, i) {
      ...
      let e = t;
      return (
        this._ttPolicy != null && (e = this._ttPolicy.createScriptURL(t)),
        (r.src = e),
        r
      );
    }
  };
  async function T(s, t, i) {
    if (t < 0) return s;
    let r,
      e = new Promise((n, a) => {
        r = setTimeout(() => {
          a(
            new Error(`${i} took more than ${t}ms to resolve. Moving on.`, {
              cause: T,
            }),
          );
        }, t);
      });
    return Promise.race([s, e]).finally(() => {
      clearTimeout(r);
    });
  }
  var v = class {
...
Read more: https://developer.mozilla.org/en-US/docs/Web/Security/Secure_Contexts`)),
      ...
      let {
        ...
        serviceWorkerUrl: r = u(`flutter_service_worker.js?v=${i}`),
        timeoutMillis: e = 4e3,
      } = t,
        n = r;
      ...
      return T(a, e, "prepareServiceWorker");
    }
    async _getNewServiceWorker(t, i) {
      if (!t.active && (t.installing || t.waiting))
        return (
          console.debug("Installing/Activating first service worker."),
          t.installing || t.waiting
        );
      if (t.active.scriptURL.endsWith(i))
        return console.debug("Loading from existing service worker."), t.active;
      {
      ...
      window.trustedTypes &&
        (this.policy = trustedTypes.createPolicy(i, {
          createScriptURL: function (e) {
            ...
            let n = new URL(e, window.location),
              a = n.pathname.split("/").pop();
            if (r.some((l) => l.test(a))) return n.toString();
            console.error(
  ...
  var I = (s, t, i, r) => (
    (window.flutterCanvasKitLoaded = (async () => {
      ...
      if (!e && t.canvasKitVariant == "chromium")
        throw "Chromium CanvasKit variant specifically requested, but unsupported in this browser";
      let n = e && t.canvasKitVariant !== "full",
        a = r;
      n && (a = u(a, "chromium"));
      let o = u(a, "canvaskit.js");
      s.flutterTT.policy && (o = s.flutterTT.policy.createScriptURL(o));
      let l = g(u(a, "canvaskit.wasm")),
        m = await import(o);
      return (
        (window.flutterCanvasKit = await m.default({ instantiateWasm: l })),
  ...
  var U = async (s, t, i, r) => {
    ...
    return await (
      ...
      locateFile: (l, m) => {
        let c = m + l;
        return c.endsWith(".worker.js")
          ? URL.createObjectURL(
            new Blob([`importScripts('${c}');`], {
              type: "application/javascript",
            }),
          )
          : c;
      },
      mainScriptUrlOrBlob: e,
    });
  };
  var k = class {
    async loadEntrypoint(t) {
      let { serviceWorker: i, ...r } = t || {},
        e = new y(),
        n = new v();
      n.setTrustedTypesPolicy(e.policy),
        await n.loadServiceWorker(i).catch((o) => {
          console.warn("Exception while loading service worker:", o);
        });
      let a = new h();
      return (
        a.setTrustedTypesPolicy(e.policy),
    ...
    async load({
      ...
      let a = (d) => {
        ...
        o = (d, C) => {
          switch (d.renderer) {
            case "auto":
              return C == "canvaskit" || C == "html";
            default:
              return d.renderer == C;
          }
        },
        l = (d) =>
          (d.compileTarget === "dart2wasm" && !p.supportsWasmGC) ||
            (e.renderer && !o(d, e.renderer))
            ? !1
            : a(d.renderer),
        m = n.builds.find(l);
      if (!m)
      ...
      (c.flutterTT = new y()),
        t &&
        ((c.serviceWorkerLoader = new v()),
          c.serviceWorkerLoader.setTrustedTypesPolicy(c.flutterTT.policy),
          await c.serviceWorkerLoader.loadServiceWorker(t).catch((d) => {
            console.warn("Exception while loading service worker:", d);
          }));
      let w = b(e, n);
      m.renderer === "canvaskit"
        ? (c.canvasKit = I(c, e, p, w))
        : m.renderer === "skwasm" && (c.skwasm = U(c, e, p, w));
      let f = new h();
      return (
        f.setTrustedTypesPolicy(c.flutterTT.policy),
        (this.didCreateEngineInitializer =
          f.didCreateEngineInitializer.bind(f)),
        f.load(m, c, e, r, i)
      );
    }
  };
  window._flutter || (window._flutter = {});
  window._flutter.loader || (window._flutter.loader = new k());
})();
...
_flutter.loader.load({
  serviceWorkerSettings: {
    serviceWorkerVersion: "2573551977",
  },
});
